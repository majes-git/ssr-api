...
class RestGraphqlApi(object):
    ...
    def __init__(
        self, host="localhost", verify=False, user="admin", password=None, app=__file__
    ):
        self.host = host
        self.verify = verify
        self.user = user
    ...
    def get_nodes(self, router_name):
        return self.get(
            "/config/running/authority/router/{}/node".format(router_name)
        ).json()
    ...
    def get_router_status(self, router_name):
        ...
        if not statuses:
            warning(
                f"No assets for router {router_name} found. This should not happen."
            )
